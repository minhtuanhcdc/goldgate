      ...
      <div class="grid grid-cols-1 mb-2">
        <div class="flex flex-1 justify-between">
            ...
            <button  class="ml-2 bg-green-600 px-2 py-0 rounded-md float-right cursor-pointer h-8 text-white">Export EXCEL<span class="text-xs ml-1">(1)</span></button>
             <!-- <button @click="multiPrint(checkPrint)"  class="ml-2 bg-green-600 px-2 py-0 rounded-md float-right cursor-pointer h-8 text-white">Multi Print<span class="text-xs ml-1">({{checkPrint?checkPrint.length:0}})</span></button> -->
            </div>
...
setup() {
  ...
  computed: {
    breadcrumbs() {
      return [
        {
          label: "Danh sách nhập thông tin",
          class: "text-white",
        },
      ];
    },
  ...
  methods:{
    ...
    multiPrint(data){
      ...
      //const ids = [1, 5];
      const newArr = fillArr.map(i => list.find(({id}) => i === id));
      console.log('Heheheheheeheheheeeeee: ',newArr);
    ...
    printResult(bill){
      console.log('Heheheheheehehehehe:',this.checkPrint);
