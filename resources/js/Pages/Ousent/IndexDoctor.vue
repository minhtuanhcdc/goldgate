<template>
  <app-layout title="User list">
    ...
    <Container>
      ...
      --->
      <Card>
           <Button  class="mb-1 float-right cursor-pointer"  @click="addDoctor">+ Add</Button>
        <Table :headers="headers" :addClass="addClass">
          <tr v-for="(dtr, i) in doctors" :key="i" class="hover:bg-blue-300">
            <td class="border-r-2">{{dtr.id}}</td>
            <td class="border-r-2">{{dtr.name}}</td>
            <td class="border-r-2">{{dtr.title}}</td>
            <td class="border-r-2">{{dtr.ousent.name}}</td>
